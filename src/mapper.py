...
def map_to_PGPR(dataset_name):
    ...
    if dataset_name == ML1M:
        ...
        with open(f'data/{dataset_name}/cleaned/tracks.tsv', 'r', encoding="latin-1") as org_movies_file:
            reader = csv.reader(org_movies_file)
            for row in reader:
                row = row[0].split("\t")
                name = ""
                if len(row) == 3:
                    name = row[2]
                org_datasetid2movie_title[row[0]] = name
        org_movies_file.close()
